...
const roleLabels: Record<Role, string> = { volunteer: "อสม.", staff: "เจ้าหน้าที่", clinician: "แพทย์", citizen: "ประชาชน", coordinator: "ผู้ประสานงาน", admin: "ผู้ดูแลระบบ" };
...
export function AppShell({ user, onUserChange, children }: { user: MockUser; onUserChange: (user: MockUser) => void; children: React.ReactNode }) {
  ...
  return <div className="app-frame">
    <aside className="desktop-rail">
      ...
      <nav className="rail-nav" aria-label="เมนูหลักบนเดสก์ท็อป">{nav.map(({ label, href, icon: Icon }) => <Link key={href} href={href} className={cn("rail-link", location === href && "is-active")}><Icon size={18} />{label}</Link>)}</nav>
    ...
    <nav className="mobile-nav" aria-label="เมนูหลักบนมือถือ">{nav.slice(0, 5).map(({ label, href, icon: Icon }) => <Link key={href} href={href} className={cn("mobile-nav-link", location === href && "is-active")}><Icon size={19} /><span>{label}</span></Link>)}</nav>
  </div>;
}
