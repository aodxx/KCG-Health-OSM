...
export type FormQuestionType = "short_text" | "long_text" | "number" | "checkbox" | "radio" | "select" | "date" | "time" | "yes_no" | "single_choice" | "multiple_choice";
...
export type SubmissionMode = CompletionMode;
export type ReviewMode = "review_all" | "review_by_rule";
export type ReviewStatus = "pending" | "reviewed" | "action_required";
...
export interface Tambon { id: EntityId; name: string; districtName?: string; provinceName?: string; active?: boolean; }
export interface ServiceUnit { id: EntityId; name: string; villages: number[]; code?: string; unitType?: string; active?: boolean; tambonId?: EntityId; }
export interface Village { id: EntityId; number: number; name: string; serviceUnitId: EntityId; active?: boolean; tambonId?: EntityId; }
...
export interface Staff { id: EntityId; userAccountId: EntityId; serviceUnitId: EntityId; staffType?: string; active: boolean; }
...
export type HouseholdAssignment = VolunteerAssignment;
...
export interface FormSection { id: EntityId; formVersionId: EntityId; title: string; description?: string; order: number; }
...
export interface AudienceDefinition { id: EntityId; campaignId: EntityId; name: string; targetType: AudienceTargetType; rules?: AudienceRule[]; }
...
export interface SubmissionReview { id: EntityId; submissionId: EntityId; reviewerUserId?: EntityId; status: ReviewStatus; noteSummary?: string; reviewedAt?: string; }
export interface SyncQueueItem<TPayload = Record<string, unknown>> { mutationId: EntityId; clientGeneratedId: EntityId; idempotencyKey: string; operation: string; payload: TPayload; state: SyncState; createdAt: string; }
